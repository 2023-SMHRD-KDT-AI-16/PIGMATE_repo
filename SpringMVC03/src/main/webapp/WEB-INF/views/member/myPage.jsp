...
<script type="text/javascript">
    ...
    function getFarm() {
        $.ajax({
            ...
            error: function(xhr, status, error) {
                ...
                alert("농장 데이터를 불러오는 중 오류가 발생했습니다. 상태: " + status + ", 오류: " + error);
            }
        });
    }
    ...
    function makeView(data) {
        ...
        $.each(data, function(index, obj) {
            ...
            listHtml += "<td><button class='btn btn-outline-primary' onclick='editFarm(this)'>수정</button> <button class='btn btn-outline-success' onclick='saveFarm(this)' style='display:none'>저장</button></td>";
    ...
    function appendFarmToList(farmData) {
        ...
        newRow += "<td><a href='javascript:goContent(env_idx)'>" + "환경 기준" + "</a></td>";
        newRow += "<td><button class='btn btn-outline-primary' onclick='editFarm(this)'>수정</button> <button class='btn btn-outline-success' onclick='saveFarm(this)' style='display:none'>저장</button></td>";
    ...
    function saveFarm(button) {
        ...
        $.ajax({
            ...
            success: function(response) {
                // 성공적으로 업데이트되면 입력 필드를 다시 readonly로 설정
    ...
    function deleteFarm(button) {
        var farmName = $(button).closest('tr').find('td:first input').val(); // 농장 이름을 가져옴
                            ...
                            <div class="card mb-0">
                                <div class="card-body p-4">
                                    ...
                                    <form action="insertEnvCri.do" method="post">
                                        <div class="form-floating mb-3">
                                            <label for="farm">Select Farm:</label> <select name="farm_idx" id="farm" class="form-control">
