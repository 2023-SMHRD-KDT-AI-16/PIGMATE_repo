	...
	<div>
		...
		<nav class="sidebar-nav scroll-sidebar" data-simplebar="">
			<ul id="sidebarnav">
				...
				<li class="sidebar-item"><a class="sidebar-link home-link"
				...
				<c:if test="${not empty sessionScope.mvo}">
					<c:set var="firstFarmId"
						value="${sessionScope.mvo.farms[0].farm_idx}" />
					<c:forEach var="farm" items="${sessionScope.mvo.farms}">
						<li class="sidebar-item">
							<div class="d-flex align-items-center justify-content-between">
								<a class="sidebar-link farm-link"
									href="${contextPath}/?farmId=${farm.farm_idx}"
									aria-expanded="false"> <iconify-icon
										icon="solar:layers-minimalistic-bold-duotone"></iconify-icon>
									<span class="hide-menu1">${farm.farm_name}</span>
								</a>
								<iconify-icon icon="mdi:chevron-down"
									class="ms-auto cursor-pointer"
									onclick="toggleSubmenu('${farm.farm_idx}')"></iconify-icon>
							</div>
							<ul id="${farm.farm_idx}" class="collapse list-unstyled">
								...
								<li class="sidebar-item"><a class="sidebar-link env-link"
									href="${contextPath}/farmEnv.do?farmId=${farm.farm_idx}"> <iconify-icon
											icon="mdi:leaf"></iconify-icon> <span class="hide-menu2">환경정보</span>
								</a></li>
								<li class="sidebar-item"><a
									class="sidebar-link report-link"
									href="${contextPath}/#.do?farmId=${farm.farm_idx}"> <iconify-icon
											icon="mdi:file-document-outline"></iconify-icon> <span
										class="hide-menu2">리포트</span>
								</a></li>
							</ul>
						</li>
					</c:forEach>
			...
			function() {
				...
				$(".sidebar-link").each(
						function() {
							...
							var pathName = new URL(linkUrl,
									window.location.origin).pathname;
							var searchParams = new URL(linkUrl,
									window.location.origin).search;
							if (currentUrl === pathName + searchParams
									&& currentUrl !== "${contextPath}/") {
								$(this).addClass("active");
								$(this).closest(".collapse").addClass("show");
								$(this).closest(".sidebar-item").find(
										"> .sidebar-link").addClass("active");
							}
						});
